...
def moments(dataset, indices):
    ...
    for index in indices:
        ...
        actions.append(sample["action"].reshape(-1, sample["action"].shape[-1]))
        if sample["tactile"].numel():
            tactile.append(sample["tactile"])
...
def main():
    ...
    val_ids, train_ids = indices[:val_count], indices[val_count:]
    if not train_ids:
        raise ValueError("Dataset needs at least two frames for train/validation split.")
    sample = dataset[train_ids[0]]
    config = DiffusionPolicyConfig(
        sample["state"].numel(), sample["tactile"].numel(), sample["action"].shape[-1], args.horizon
    )
    model = DiffusionPolicy(config).to(args.device)
    stats = moments(dataset, train_ids)
    model.set_normalization(*(value.to(args.device) for value in stats))
    train_loader = DataLoader(
        Subset(dataset, train_ids), batch_size=args.batch_size, shuffle=True, num_workers=0
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        with torch.no_grad():
            for batch in val_loader:
                batch = {k: v.to(args.device) for k, v in batch.items()}
                val_loss += model.loss(**batch).item() * batch["state"].shape[0]
        ...
        if val_loss < best:
            ...
            payload.update({"epoch": epoch, "val_loss": best, "repo_id": args.repo_id})
