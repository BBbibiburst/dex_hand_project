...
| PickPlace 自动迁移 | 复用 Lift 的完整 6D 初态/验证终态；目标中心为 `(0.46, 0.20)`，目标以桌面彩色区域表示 |
...
PickPlace 下降会重新测量运输后的手腕—物体偏移，并以手部碰撞几何到桌面的安全距离限制
最低手腕高度。释放采用渐进卸力；若空中横移无法保持球形或边缘抓取，则自动切换为低速
桌面支撑搬运，最后才使用从 reset 开始的连续 Lift 控制重放。源区和目标区是贴在真实桌面
上的可视标记，不使用无碰撞假箱壁。
