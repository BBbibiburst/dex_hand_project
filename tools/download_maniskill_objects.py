...
def download_egad(cache_dir: Path, *, force: bool) -> None:
    ...
    expected_paths = {object_id: target / object_id / f"{object_id}.obj" for object_id in EGAD_EVAL_IDS}
...
def contains_model_files(path: Path) -> bool:
    return any(
        item.is_file() and item.suffix.lower() in MODEL_SUFFIXES for item in path.rglob("*")
    )
...
def discover_object_ids(cache_dir: Path, spec: DatasetSpec) -> list[str]:
    ...
    object_ids = [
        ...
        if path.is_dir() and contains_model_files(path)
    ]
    if not object_ids:
...
def choose_ycb_objects(
    available: list[str], selection: Path | None, count: int
) -> list[str]:
    if count < 0:
...
def write_lock_file(path: Path, dataset: str, object_ids: Iterable[str]) -> None:
    ...
    lines = [
        f"# Generated by tools/download_maniskill_objects.py",
...
def migrate_legacy_output(output_dir: Path, manifest_path: Path) -> None:
    ...
    if manifest_path == DEFAULT_MANIFEST and not manifest_path.exists() and legacy_manifest.is_file():
...
def main() -> int:
    ...
    for dataset_name in args.datasets:
        ...
        print(
            f"[plan] {dataset_name}: selected {len(selected)} of {len(available)} "
            "available objects"
        )
