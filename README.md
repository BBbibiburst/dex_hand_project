...
`--viewer` 和 `--preview-image` 使用轻量点云显示物体、手指语义区域、接触点以及两段
轨迹：绿色为完全张开手型的无接触 approach，红色为经过检查的预成形/闭合抓取轨迹。
目前支持六维 `dex_hand` 和单自由度 `pika_gripper`。程序从物体表面锚点和局部法向生成
位姿候选，联合搜索手腕相对位姿、闭合量和接触点。Dex Hand 使用完整多指六维 wrench
约束；Pika 使用适合双指夹爪的对向接触、重力支撑和双指闭合约束。

发布前会在独立 MuJoCo 手-物体模型中逐 waypoint 验证 approach：自由空间段不允许任何
物体接触，抓取段只允许指定手指/夹爪接触，掌部和基座碰撞会淘汰该候选并继续尝试下一
候选。无效搜索默认不写配置；只有显式指定 `--output` 的调试运行才可保存失败结果。

```bash
# 使用资产清单中的物体
python -m tools.grasping.search_grasp \
  --object-id ycb:005_tomato_soup_can \
  --output configs/grasps/dex_hand/ycb_005_tomato_soup_can.json \
  --preview-image configs/grasps/dex_hand/ycb_005_tomato_soup_can.png
...
| `--joint-candidates N` | `128` | 末端手型/开口候选预算 |
| `--surface-anchors N` | `24` | 物体表面抓取锚点数量 |
| `--rolls-per-anchor N` | `8` | 每个锚点的手腕滚转候选数量 |
| `--seed N` | `0` | 随机种子，用于复现实验 |
| `--target-size M` | `0.09` | 将物体最长边归一化到该尺寸，单位为米 |
| `--end-effector NAME` | `dex_hand` | 选择 `dex_hand` 或 `pika_gripper` |
| `--output PATH` | 按末端自动选择 | 写入 `configs/grasps/<end_effector>/` |
| `--preview PATH` | 无 | 用 Trimesh 导出物体、手和接触点组成的 3D 场景，如 `.glb` |
| `--preview-image PATH` | 无 | 保存物体/手点云、接触点和两段轨迹的 PNG 预览图 |
...
`source.grasping.grasp_config_search` 是正式的唯一搜索实现，负责物体点云、末端手型、
抓取候选、两段轨迹、版本化配置和发布前碰撞验证。Matplotlib 仅在请求预览时延迟加载。
...
python -m tools.grasping.benchmark_catalog --jobs 8
...
python -m tools.grasping.benchmark_catalog --dataset egad --limit 10
...
```text
configs/grasps/dex_hand/<object>.json
configs/grasps/pika_gripper/<object>.json
```
...
在完整任务环境中运行脚本策略，但不记录数据。Lift 策略为
`approach → grasp → lift → verify`：approach 保持手完全张开并到达预抓取位姿，grasp
沿经过碰撞检查的预成形/前移轨迹闭合，最后独立保持验证。Viewer 中会显示末端目标位姿、
...
```bash
python -m tools.grasping.validate_scripted_strategy --task lift
...
| 程序 | 功能 | 常用参数和运行示例 |
| --- | --- | --- |
| `tools.run_smoke_checks` | 无窗口检查机器人模型、控制、触觉和注册任务，失败时返回非零状态码 | `python -m tools.run_smoke_checks --steps 2`；`--skip-tasks` 跳过任务，`--fail-fast` 首次失败即停止 |
| `examples.manipulation_task_playback` | 在 Viewer 中查看任务、物体随机化和手动/预设动作 | `python -m examples.manipulation_task_playback --task lift --object-id ycb:005_tomato_soup_can`；完整选项见 `--help` |
| `examples.robot_preview` | 预览当前机器人和关节控制 | `python -m examples.robot_preview`；`--no-scene` 只加载机器人 |
| `examples.ik_trajectory` | 让末端沿椭圆轨迹运动，用于检查 IK、坐标系和控制平滑度 | `python -m examples.ik_trajectory --frequency 0.08 --radius-x 0.1 --radius-y 0.1`；`--no-realtime` 最快运行 |
| `examples.random_control` | 向 position/IK 控制器发送可复现随机动作 | `python -m examples.random_control --seed 0`；`--no-realtime` 取消实时限速 |
...
| 程序 | 功能 | 运行方式和关键参数 |
| --- | --- | --- |
| `tools.tactile.preview_layout` | 显示 taxel 布局、编号和表面方向 | `python -m tools.tactile.preview_layout --help` |
...
| 程序 | 功能 | 运行方式 |
| --- | --- | --- |
| `apps.collect_teleop_lerobot` | 用 Vive 位姿和蓝牙弯曲手套收集 LeRobotDataset | `python -m apps.collect_teleop_lerobot --task lift --episodes 10 --output datasets/lerobot` |
...
`python -m apps.collect_teleop_lerobot --help` 输出为准。
...
```powershell
python -m tools.run_smoke_checks --skip-tasks
```
...
python -m examples.manipulation_task_playback --task lift --object-id egad:G6
...
python -m examples.manipulation_task_playback --task nut_assembly
...
```powershell
python -m examples.manipulation_task_playback `
  --task pick_place `
...
```powershell
python -m tools.tactile.validate_contacts `
  --robot-config configs/robot_profiles/rm75b_pika_gripper.json
```
