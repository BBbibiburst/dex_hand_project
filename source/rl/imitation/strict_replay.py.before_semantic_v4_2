...
Two validation profiles are intentionally separated:
...
_PROFILE_DEFAULTS = {
    "expert": {
        ...
        "verify_tail": 20,
        "extra_hold_steps": 8,
        "maximum_penetration": 0.004,
        "minimum_tail_contact_fraction": 0.70,
...
def _contact_sets(env):
    ...
    palm_geom = mujoco.mj_name2id(env.model, mujoco.mjtObj.mjOBJ_GEOM, "skin_palm_p")
...
def strict_replay_manifest(
    ...
    *,
    ...
    try:
        ...
        controls, initial_qpos, initial_qvel, initial_ctrl = _load_reference_for_replay(
            manifest, env
        )
