...
class BCHandPolicy(nn.Module):
    """Predict six normalized physical hand actuator commands."""
...
class BCTrainConfig:
    ...
    reference_noise_std: float = 0.08
    ...
    def validate(self) -> None:
        ...
        if self.reference_noise_std < 0.0:
            raise ValueError("reference_noise_std must be non-negative.")
...
def collect_bc_dataset(
    ...
    *,
    ...
    """Replay strict expert references and record geometry-aware state->hand targets."""
    ...
    np.savez_compressed(
        ...
        object_indices=np.asarray(object_indices, dtype=np.int32),
    )
    info = BCDatasetInfo(
...
def train_bc_policy(
    ...
    *,
    ...
    object_indices = np.asarray(arrays["object_indices"], dtype=np.int64)
    ...
    reference_slice = feature_slices.get("reference_hand")
    if not (
        ...
        raise ValueError("BC dataset lacks a valid reference_hand observation slice.")
    ref_lo, ref_hi = map(int, reference_slice)

    unique_objects = np.unique(object_indices)
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        for start in range(0, len(permutation), cfg.batch_size):
            ...
            if cfg.reference_noise_std > 0.0:
                noise = torch.randn_like(obs[:, ref_lo:ref_hi]) * cfg.reference_noise_std
    ...
    print(
        ...
        f"validation_objects={validation_object_ids}",
        flush=True,
    )
    return {
        "final_train_loss": final_train,
