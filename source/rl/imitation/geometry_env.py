...
class GeometryAwareResidualLiftEnv(MjWarpResidualLiftEnv):
    ...
    def _prepare_geometry_context(self) -> None:
        ...
        def resolve_geom_id(local_name: str) -> int:
            """Resolve a hand-local geom name after MjSpec prefixing.

            Attached Dex Hand entities are compiled as e.g.
            ``dexhand_skin_0_2_p``.  The geometry observation is specified in
            local hand names, so always respect the controller's actual prefix
            and keep a suffix fallback for custom profiles.
            """
            candidates = []
            if hand_prefix:
        ...
        def geom_context(name: str) -> tuple[int, int, torch.Tensor]:
            geom = resolve_geom_id(name)
            if geom < 0:
                return -1, -1, torch.zeros(3, device=self.torch_device)
            body = int(self.model.geom_bodyid[geom])
            local = torch.as_tensor(
                np.asarray(self.model.geom_pos[geom], dtype=np.float32),
                device=self.torch_device,
            )
            return int(geom), body, local
        ...
        if self.palm_geom_id < 0 or any(value < 0 for value in self.fingertip_geom_ids):
            missing = [
                name
                for name, geom in [
                    ("skin_palm_p", self.palm_geom_id),
                    *[(f"skin_{d}_2_p", self.fingertip_geom_ids[d]) for d in range(5)],
                ]
                if geom < 0
            ]
    ...
    def _geom_center_world(
        ...
        body_position = self.xpos[:, body_id]
        body_quaternion = self.xquat[:, body_id]
        local = local_position.view(1, 3).expand(self.num_envs, -1)
        return body_position + _quat_rotate(body_quaternion, local)

    def _fingertip_world_positions(self) -> torch.Tensor:
        return torch.stack(
            [
                self._geom_center_world(body, local)
                for body, local in zip(
                    self.fingertip_body_ids,
                    self.fingertip_local_positions,
                )
            ],
            dim=1,
        )
    ...
    def _geometry_features(self) -> torch.Tensor:
        ...
        palm_position = self._geom_center_world(
            self.palm_body_id,
            self.palm_local_position,
        )
