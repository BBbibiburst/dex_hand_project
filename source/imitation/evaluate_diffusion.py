...
def parse_args():
    ...
    p.add_argument("--task", choices=registered_tasks(), required=True)
...
def main():
    ...
    try:
        for episode in range(args.episodes):
            ...
            while steps < args.max_steps and not success:
                ...
                if (
                    ...
                    raise ValueError(
                        "Evaluation robot/task observation dimensions do not match the training checkpoint."
                    )
        ...
        env.close()
    print(
        f"success_rate={successes / args.episodes:.3f} ({successes}/{args.episodes}) mean_return={np.mean(returns):.3f} mean_steps={np.mean(lengths):.1f}"
    )
