...
class VisionEncoder(nn.Module):
    def __init__(self, output_dim):
        ...
        self.network = nn.Sequential(
            nn.Conv2d(3, 32, 5, stride=2, padding=2), nn.GroupNorm(4, 32), nn.SiLU(),
            ...
            nn.AdaptiveAvgPool2d(1), nn.Flatten(), nn.Linear(128, output_dim), nn.SiLU(),
        )

    def forward(self, image): return self.network(image)
...
class DiffusionPolicy(nn.Module):
    def __init__(self, config: DiffusionPolicyConfig):
        ...
        self.tactile = (
            ...
            if config.tactile_dim else None
        )
        ...
        self.denoiser = nn.Sequential(
            nn.Linear(input_dim + 2 * d, 1024), nn.SiLU(),
            nn.Linear(1024, 1024), nn.SiLU(), nn.Linear(1024, input_dim),
        )
    ...
    def set_normalization(self, state_mean, state_std, tactile_mean, tactile_std,
                          action_mean, action_std):
        self.state_mean.copy_(state_mean); self.state_std.copy_(state_std.clamp_min(1e-6))
        if self.config.tactile_dim:
            self.tactile_mean.copy_(tactile_mean); self.tactile_std.copy_(tactile_std.clamp_min(1e-6))
        self.action_mean.copy_(action_mean); self.action_std.copy_(action_std.clamp_min(1e-6))
    ...
    def from_checkpoint(cls, checkpoint, device="cpu"):
        ...
        model.load_state_dict(checkpoint["model"]); return model.to(device)
