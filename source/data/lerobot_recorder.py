...
class LeRobotEpisodeRecorder:
    def __init__(
        ...
        *,
        ...
        include_operator_metadata: bool = True,
    ) -> None:
        try:
        ...
        self.include_operator_metadata = bool(include_operator_metadata)
        features = {
            STATE_KEY: {
                "dtype": "float32",
                "shape": (state_dim,),
        ...
        if self.include_operator_metadata:
            features[OPERATOR_GLOVE_KEY] = {
                "dtype": "float32",
                "shape": (6,),
                "names": None,
            }
            features[OPERATOR_VIVE_POSE_KEY] = {
                "dtype": "float32",
                "shape": (7,),
                "names": None,
            }
    ...
    def add_frame(
        ...
        *,
        ...
        if self.include_operator_metadata:
            if glove is None or vive is None:
                raise ValueError("Operator metadata is enabled, but glove or Vive data is missing.")
            frame[OPERATOR_GLOVE_KEY] = np.asarray(glove.stretch, dtype=np.float32)
            frame[OPERATOR_VIVE_POSE_KEY] = np.concatenate(
