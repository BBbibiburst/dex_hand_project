# -*- coding: utf-8 -*-
"""Manipulation tasks and environment factories.

Built-in tasks are imported through one explicit module. Third-party tasks can
still register themselves with :func:`register_task`.
"""

from source.envs.core.registry import make_task, register_task, registered_tasks
from source.envs.manipulation.arenas import BinsArena, PegsArena, PushArena, TableArena
from source.envs.manipulation.base import SingleArmManipulationTask
from source.envs.manipulation.factory import (
    make_lift_env,
    make_manipulation_env,
    make_nut_assembly_env,
    make_pick_place_env,
    make_push_env,
    make_stack_env,
)
from source.envs.manipulation.objects import (
    FreeBoxSpec,
    FreeCylinderSpec,
    FreeNutSpec,
    ManipulationObjectSpec,
    MeshObjectSpec,
    XmlNutSpec,
)
from source.envs.manipulation.placement import PlacementRegion, UniformTablePlacementSampler

__all__ = [
    "BinsArena",
    "FreeBoxSpec",
    "FreeCylinderSpec",
    "FreeNutSpec",
    "ManipulationObjectSpec",
    "MeshObjectSpec",
    "PegsArena",
    "PlacementRegion",
    "PushArena",
    "SingleArmManipulationTask",
    "TableArena",
    "UniformTablePlacementSampler",
    "XmlNutSpec",
    "make_lift_env",
    "make_manipulation_env",
    "make_nut_assembly_env",
    "make_pick_place_env",
    "make_push_env",
    "make_stack_env",
    "make_task",
    "register_task",
    "registered_tasks",
]
